...
exports.newParkingSuggestion = async (lat, lon, name, freeText, userEmail) => {
    ...
    if (regions.length > 0) {
        ...
        await EM.dispatchNewParkingSuggestionToRegionReps(reps, location, regions[regions.length - 1], name, freeText, userEmail);
...
exports.parkingCorrectionSuggestion = async (parkingUri, suggestion, userEmail) => {
    ...
    if (parking) {
        ...
        await EM.dispatchParkingCorrectionSuggestion(reps, parkingUri, localId, suggestion, userEmail);
        return reps.map(rep => rep.email);
    } else {
        return null;
    }
